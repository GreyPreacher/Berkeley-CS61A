class VendingMachine:
    ...
    def restock(self, stock_num):
        if stock_num <= 0:
            print('The stock num is less than 0, Please restock')
        else:
    ...
    def vend(self):
        ...
        self.stock -= 1
        self.balance = 0
        return f'Here is your {self.type} and ${change} change.'
...
def bst_min(t):
    if t.is_leaf():
        return t.label
    return min(t.label, min([bst_min(branch) for branch in t.branches]))

def bst_max(t):
    if t.is_leaf():
        return t.label
    return max(t.label, max([bst_min(branch) for branch in t.branches]))
...
def is_bst(t):
    ...
    if len(t.branches) == 1:
        return is_bst(left_child) and (t.label >= bst_max(left_child) or
                                       t.label <= bst_min(left_child))
    right_child = t.branches[1]
    return is_bst(left_child) and is_bst(right_child) \
           and t.label >= bst_max(left_child) and t.label <= bst_min(right_child)
