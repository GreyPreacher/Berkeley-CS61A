...
def two_of_three(x, y, z):
    ...
    50
    """
    return add(min(x,y,z)*min(x,y,z),max(min(x,y),min(x,z),min(y,z))*max(min(x,y),min(x,z),min(y,z)))
...
def invert_short(x, limit):
    ...
    100
    """
    return limited(x, 1 / x if x!=0 else 0, limit)
...
def change_short(x, y, limit):
    ...
    100
    """
    return limited(x, abs(y - x) / x if x!=0 else 0, limit)
