...
def count_coins(change):
    ...
    True
    """
    def count_change(change,max_coin=25):
        if change<5:
            return 1
        elif change<10:
            return 2
        if change<max_coin:
            return count_change(change,descending_coin(max_coin))
        if max_coin==1:
            return 1
        return count_change(change-max_coin,max_coin)+count_change(change,descending_coin(max_coin))
    return count_change(change)
...
def move_stack(n, start, end):
    ...
    Move the top disk from rod 1 to rod 3
    """
    # Hanoi tower, you can find a picture-noted explain on CSDN
