...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        dist = 0
        while (not near_place.is_hive) and near_place != None \
                and dist <= self.max_range:
            if len(near_place.bees) != 0 and dist >= self.min_range:
...
# The WallAnt class
class WallAnt(Ant):
    '''WallAnt safeguard our basement'''
    ...
    #A WallAnt does nothing each turn
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant):
        # BEGIN Problem 8
        assert self.ant_contained is None
