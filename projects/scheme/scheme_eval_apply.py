...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        # BEGIN PROBLEM 2
        def transfer_scm_to_py(a):
            if a is nil:
                return []
            return [a.first] + transfer_scm_to_py(a.rest)
        transfered = transfer_scm_to_py(args)
        if procedure.expect_env:
            transfered.append(env)
        try:
            return procedure.py_func(*transfered)
        except TypeError:
